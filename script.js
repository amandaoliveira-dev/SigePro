document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateUI = () => {
        ...
        let subtotal = 0;
        cart.forEach((item, index) => { const totalItem = item.preco * item.quantidade; subtotal += totalItem; const row = document.createElement('tr'); if (item.isDiscount) row.className = 'discount-item'; row.innerHTML = `<td>${item.codigo}</td><td>${toTitleCase(item.nome)}</td><td>${formatCurrency(item.preco)}</td><td>${item.quantidade}</td><td>${formatCurrency(totalItem)}</td><td><button class="remove-btn" data-index="${index}">✖</button></td>`; cartItemsContainer.appendChild(row); });
